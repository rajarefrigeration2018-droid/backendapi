...
def create_address(body: AddressIn, user=Depends(get_current_user)):
    ...
    if not area or not area["is_active"]:
        fail("We don't serve this pincode yet.", "NOT_SERVICEABLE")
...
def delete_address(aid: int, user=Depends(get_current_user)):
    ...
    if active:
        fail("This address has an active booking.", "ADDRESS_IN_USE")
...
def create_booking(body: BookingCreateIn, user=Depends(get_current_user)):
    ...
    if body.scheduled_date < today:
        fail("You can't book a date in the past.", "PAST_DATE")
    ...
    if body.scheduled_date > today + timedelta(days=max_days):
        fail(f"You can only book up to {max_days} days in advance.", "TOO_FAR")
    ...
    if not slot:
        fail("This slot is not available.", "SLOT_NOT_FOUND", 404)

    if body.scheduled_date == today and slot["start_time"] <= datetime.now().time():
        fail("This slot has already passed. Please pick another one.", "SLOT_PASSED")
    ...
    if taken and int(taken) >= slot["max_bookings"]:
        fail("This slot is full. Please pick another one.", "SLOT_FULL")
    ...
    if body.coupon_code and not breakup.coupon_applied:
        fail(breakup.coupon_message or "Coupon could not be applied", "COUPON_INVALID")
    ...
    if body.payment_mode in ("cod", "wallet") or fully_paid_by_wallet:
        ...
        # kick off auto-assignment to nearby mechanics
        try:
            assignment.offer_to_partners(booking["id"])
        except Exception:  # noqa: BLE001
            logger.exception("Auto-assign failed for booking %s", booking["id"])
...
def cancel_booking(bid: int, body: CancelIn, user=Depends(get_current_user)):
    ...
    if not info["can_cancel"]:
        fail(info["note"] or "This booking can't be cancelled.", "CANNOT_CANCEL")
    ...
    return ok(
        ...
        "Booking cancelled." + (f" A cancellation fee of ₹{fee:.0f} was applied." if fee else ""),
    )
...
def reschedule_booking(bid: int, body: RescheduleIn, user=Depends(get_current_user)):
    ...
    if booking["status"] not in ("pending", "confirmed", "assigned"):
        fail("This booking can no longer be rescheduled.", "CANNOT_RESCHEDULE")

    if body.scheduled_date < datetime.now().date():
        fail("You can't pick a past date.", "PAST_DATE")
    ...
    if taken and int(taken) >= slot["max_bookings"]:
        fail("This slot is full.", "SLOT_FULL")
    ...
    try:
        ...
        logger.exception("Re-assign failed for booking %s", bid)

    return ok(None, "Booking rescheduled. A technician is being assigned.")
...
def respond_extra_charge(
    ...
    if charge["approved_by_user"] or charge["rejected"]:
        fail("You have already responded to this charge.", "ALREADY_RESPONDED")
    ...
    return ok(
        ...
        "Extra charge approved." if body.approve else "Extra charge rejected.",
    )
...
def add_review(bid: int, body: ReviewIn, user=Depends(get_current_user)):
    ...
    if booking["status"] not in ("completed", "paid"):
        fail("You can review only after the job is completed.", "NOT_COMPLETED")
    if db.fetch_one("select 1 from reviews where booking_id = :b", {"b": bid}):
        fail("You have already submitted a review.", "ALREADY_REVIEWED")
    ...
    return ok(review, "Thanks for your feedback!")
...
def _cancellation_info(booking: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if status in ("completed", "paid"):
        return {"can_cancel": False, "fee": 0.0, "note": "The job is already completed."}
    if status in ("cancelled", "rejected"):
        return {"can_cancel": False, "fee": 0.0, "note": "This booking is already cancelled."}
    if status == "in_progress":
        return {"can_cancel": False, "fee": 0.0, "note": "Work has already started."}
    ...
    return {
        "can_cancel": True,
        "fee": penalty,
        "note": f"A technician has been assigned, so a ₹{penalty:.0f} cancellation fee applies.",
    }
