...
def apply_coupon(
    ...
    if not coupon["is_active"]:
        return 0.0, None, "This coupon is no longer valid"

    now = datetime.now(timezone.utc)
    if coupon["valid_from"] and coupon["valid_from"] > now:
        return 0.0, coupon, "This coupon is not active yet"
    if coupon["valid_to"] and coupon["valid_to"] < now:
        return 0.0, coupon, "This coupon has expired"

    if coupon["usage_limit"] is not None and coupon["used_count"] >= coupon["usage_limit"]:
        return 0.0, coupon, "This coupon has reached its usage limit"

    min_order = _num(coupon["min_order"])
    if subtotal < min_order:
        short = min_order - subtotal
        return 0.0, coupon, f"Add ₹{short:.0f} more to use this coupon"
    ...
    if user_id:
        ...
        if used and int(used) >= (coupon["per_user_limit"] or 1):
            return 0.0, coupon, "You have already used this coupon"
        ...
        if coupon["first_order_only"]:
            ...
            if prior and int(prior) > 0:
                return 0.0, coupon, "This coupon is valid on your first booking only"
    ...
    if allowed_services or allowed_cats:
        ...
        if eligible_amount <= 0:
            return 0.0, coupon, "This coupon is not valid on the selected services"
    ...
    if discount <= 0:
        return 0.0, coupon, "No discount applies to this order"

    return discount, coupon, f"You saved ₹{discount:.0f}!"
